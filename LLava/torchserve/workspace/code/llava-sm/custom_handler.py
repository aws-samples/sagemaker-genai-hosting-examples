        ...
        class StoppingCriteria(object):
            ...
            def __call__(self, res):
                ...
                if self.outer.context.cache[seq_id]["end"]:
                    ...
                    logger.info(f"MYLOGS-HANDLER: end sequence_id={self.seq_id}, ret={ret}")
                    return ret
                # cancel
                    ...
                    logger.info(
                        f"MYLOGS-HANDLER: cancel sequence_id={self.seq_id}, request_id={self.req_id}, ret={ret}"
                    )
                ...
                # start
                    ...
                    self.outer.clean_up(self.seq_id, self.req_id, False)
                    logger.info(
                        f"MYLOGS-HANDLER: start sequence_id={self.seq_id}, request_id={self.req_id}, ret=None"
                    )
                ...
                # non stream
                    ...
                    self.outer.clean_up(self.seq_id, self.req_id, False)
                    logger.info(
                        f"MYLOGS-HANDLER: test non stream sequence_id={self.seq_id}, request_id={self.req_id}, ret=None"
                    )
                    return None
                else:
                    logger.info(
                        f"MYLOGS-HANDLER: continue sequence_id={self.seq_id}, request_id={self.req_id}, ret=False"
                    )
